...
def should_resize_image(width: int, height: int, max_width: int = 1200, max_height: int = 1600) -> bool:
    """Determine if image should be resized based on dimensions."""
    return width > max_width or height > max_height


def resize_image(image_path: Path, output_path: Path, max_width: int = 1200, max_height: int = 1600, quality: int = 85) -> bool:
...
def convert_to_modern_format(image_path: Path, output_path: Path, target_format: str = 'webp', quality: int = 85) -> bool:
...
def process_image(
    ...
    if success:
        ...
        if not quiet:
            ...
            print(f"Image {action_str}: {image_path.name} ({original_size:.2f} → {new_size:.2f} MB, {reduction:.1f}% reduction)")
