...
class OnixExporter:
    ...
    def _metadata_to_onix_record(self, metadata: EpubMetadata, **kwargs) -> OnixRecord:
        ...
        record_ref = (
            metadata.isbn
            or metadata.uuid
            or f"docx2shelf-{datetime.now().strftime('%Y%m%d%H%M%S')}"
        )
        ...
        # Handle contributors
        if metadata.author:
            record.contributors.append(OnixContributor(role="A01", name=metadata.author))  # Author
        ...
        for bisac_code in bisac_codes:
            if isinstance(bisac_code, str):
                record.subjects.append(
                    OnixSubject(scheme="10", code=bisac_code)  # BISAC Subject Heading
                )
        ...
        if all_keywords:
            record.subjects.append(
                OnixSubject(scheme="20", heading="; ".join(all_keywords))  # Keywords
            )
    ...
    def _create_product_element(self, record: OnixRecord) -> ET.Element:
        ...
        for contributor in record.contributors:
            ...
            ET.SubElement(contrib_elem, "SequenceNumber").text = str(
                record.contributors.index(contributor) + 1
            )
        ...
        if record.audience_age_from is not None or record.audience_age_to is not None:
            ...
            if record.audience_age_from is not None:
                ET.SubElement(audience, "AudienceCodeValue").text = (
                    f"{record.audience_age_from:02d}"
                )
            ...
            if record.audience_age_to is not None:
                ...
                ET.SubElement(range_value, "AudienceRangeValue").text = str(
                    record.audience_age_from or 0
                )
...
def export_onix_from_metadata(
    metadata: EpubMetadata, output_path: Path, config: Optional[OnixConfig] = None, **kwargs
) -> Path:
    """
