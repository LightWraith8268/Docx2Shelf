...
class PublishingChecker:
    ...
    def add_issue(
        self, severity: str, category: str, message: str, fix_suggestion: Optional[str] = None
    ):
    ...
    def check_cover_image(self, cover_path: Path) -> None:
        ...
        if not cover_path or not cover_path.exists():
            self.add_issue(
                "error", "cover", "Cover image file not found", "Provide a cover image file"
            )
            return
    ...
    def run_checks(
        self, metadata: EpubMetadata, cover_path: Optional[Path] = None
    ) -> ChecklistResult:
        ...
        return ChecklistResult(
            store=self.store_name, issues=self.issues.copy(), passed=not has_errors
        )
...
class KDPChecker(PublishingChecker):
    ...
    def _check_cover_dimensions(self, width: int, height: int) -> None:
        ...
        if width < 1600 or height < 2560:
            self.add_issue(
                "info", "cover", "For best quality, KDP recommends 1600x2560px or higher"
            )
...
class AppleChecker(PublishingChecker):
    ...
    def _run_store_specific_checks(self, metadata: EpubMetadata) -> None:
        ...
        if not metadata.copyright_holder and not metadata.rights:
            self.add_issue("info", "metadata", "Copyright information recommended for Apple Books")

        # Publisher information recommended
        if not metadata.publisher:
            self.add_issue(
                "info", "metadata", "Publisher information recommended for professional appearance"
            )
...
class KoboChecker(PublishingChecker):
    ...
    def _check_cover_dimensions(self, width: int, height: int) -> None:
        ...
        if width < 1600 or height < 2400:
            self.add_issue(
                "info", "cover", "Kobo recommends 1600x2400px or higher for best quality"
            )
...
def run_all_checklists(
    metadata: EpubMetadata, cover_path: Optional[Path] = None
) -> Dict[str, ChecklistResult]:
