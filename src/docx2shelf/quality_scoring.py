...
class EPUBQualityAnalyzer:
    ...
    def analyze_epub(self, epub_path: Path) -> QualityReport:
        ...
        try:
            with ZipFile(epub_path, "r") as epub_zip:
                ...
                report.category_scores[QualityCategory.STRUCTURE] = self._analyze_structure(
                    epub_zip
                )
                ...
                report.category_scores[QualityCategory.ACCESSIBILITY] = self._analyze_accessibility(
                    epub_zip
                )
                report.category_scores[QualityCategory.TECHNICAL] = self._analyze_technical(
                    epub_zip
                )
                report.category_scores[QualityCategory.PRESENTATION] = self._analyze_presentation(
                    epub_zip
                )
                ...
                report.total_issues = sum(
                    len(cat.issues) for cat in report.category_scores.values()
                )
    ...
    def _calculate_overall_score(
        self, category_scores: Dict[QualityCategory, CategoryScore]
    ) -> float:
    ...
    def _generate_overall_recommendations(self, report: QualityReport) -> List[str]:
        ...
        if report.quality_level == QualityLevel.CRITICAL:
            recommendations.append(
                "🚨 Critical issues detected. Address technical and metadata problems first."
            )
        ...
        if report.category_scores:
            ...
            if lowest_category[1].score < 70:
                recommendations.append(
                    f"🎯 Focus on improving {lowest_category[0].value} (lowest score: {lowest_category[1].score:.1f}%)."
                )

        if report.auto_fixable_issues > 0:
            recommendations.append(
                f"🔧 {report.auto_fixable_issues} issues can be automatically fixed."
            )
        ...
        if not recommendations:
            recommendations.append(
                "✅ Excellent quality! Consider minor optimizations for perfect score."
            )
